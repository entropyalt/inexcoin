...
#if defined(NDEBUG)
# error "inexcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x87e88658fcb8ba210f0debdd3d818b5f6b58339d613ce05072a37cdc30fd92cb");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // inexcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "inexcoin Signed Message:\n";
...
{
    // inexcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // inexcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // inexcoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // inexcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // inexcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // inexcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x85d1d1b400e037bc171529cf249153913bcf0524ee09459f7ea1e8c2920498bf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "15/09/2018 Storm Florence: Warnings of 'catastrophic' flash flooding";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 730 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04932092f542b5fc150f93eee08db9374570b12bf3696ce0f6a50f372295b7a0b1b73bb424fbc8fba38705c730b384f52f3e61653ae7f44bf0f0f411d134f11d6a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1537019237;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085698103;

        if (fTestNet)
        {
            block.nTime    = 1537019229;
            block.nNonce   = 385309631;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb28f6700f62fdd30e9f15650796190dae40fdaf8335f5cfccf27757de37f30c0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // inexcoin: increase each by adding 4 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("inexcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("inexcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static inexcoinMiner(CWallet *pwallet)
{
    printf("inexcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("inexcoin-miner");
    ...
    try { loop {
        ...
        printf("Running inexcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("inexcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&inexcoinMiner, pwallet));
}
